...
lista *addright(lista *L,int value) {
    if (isempty(L)) {
        ...
        newone->info=value;
        L->current=L->finalist=L->start=L->current->next=L->finalist->next=L->start->next=newone;
        L->length++;
        return L;
    }
...
int main() {
    ...
    if(searchvalue(&L,2)) L=addleft(L,0);
    show(L); // 1 2 0 3
    if(searchvalue(&L,1)) L=addleft(L,0);
    show(L); // 1 0 2 0 3
    if(searchvalue(&L,456)) L=addleft(L,-1);
    show(L); // 1 0 -1 2 0 3
    if(searchvalue(&L, 1)) L=delleft(L);
    show(L);
    if(searchvalue(&L, 0)) L=delleft(L);
    show(L);
    if(searchvalue(&L, 1)) L=delright(L);
    show(L);
    if(searchvalue(&L, 2)) L=delright(L);
    show(L);
    return 0;
}
